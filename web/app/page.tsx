  ...
  return (
    <header className="sticky top-0 z-30 border-b-2 backdrop-blur-sm bg-[rgba(244,241,232,0.85)]" style={{ borderColor: "#0B3D91" }}>
      <div className="mx-auto flex max-w-6xl items-stretch justify-between" style={{ minHeight: "60px" }}>
        <Link href="/" className="flex items-center gap-3 px-5 sm:px-7 border-r-2" style={{ borderColor: "#0B3D91" }}>
          <span className="font-mono font-bold text-2xl tracking-widest" style={{ color: "#0B3D91" }}>TORU</span>
        </Link>
        <div className="hidden md:flex flex-1 items-center px-6 font-mono text-[10px] tracking-wider" style={{ color: "#0B3D91" }}>
          <span className="opacity-50 mr-2">DOC.</span><span className="font-bold">TR-HOME</span>
          <span className="mx-5 opacity-25">|</span>
          <span className="opacity-50 mr-2">REV.</span><span className="font-bold">0.3 BETA</span>
          <span className="mx-5 opacity-25">|</span>
          <span className="opacity-50 mr-2">DATE</span><span className="font-bold">{RAW_DATE}</span>
        </div>
        <div className="flex items-stretch">
          <Link href="/login" className="hidden sm:flex items-center px-5 border-l-2 font-mono text-xs tracking-widest hover:bg-[rgba(11,61,145,0.06)] transition" style={{ borderColor: "#0B3D91", color: "#0B3D91" }}>
            LOGIN
          </Link>
          <Link href="/signup" className="flex items-center px-5 sm:px-7 border-l-2 font-mono text-xs tracking-widest font-bold hover:opacity-90 transition" style={{ borderColor: "#0B3D91", background: "#0B3D91", color: "#F4F1E8" }}>
            START FREE →
          </Link>
        </div>
      </div>
...
/* ============ Section ラッパー ============ */
function Section({ num, meta, anchor, children }: { num: string; meta: string; anchor: string; children: React.ReactNode; }) {
  return (
    ...
    <section className="relative pt-12 sm:pt-20 pb-12 min-h-[80vh]">
      ...
      <div className="relative">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="font-mono text-[10px] tracking-widest opacity-60" style={{ color: "#0B3D91" }}>PROJECT TORU</span>
        </div>

        <h1 className="font-bold leading-[1.12] tracking-tight bp-rise" style={{ color: "#0B3D91", fontSize: "clamp(2.5rem, 6vw, 4.25rem)" }}>
          図面を投げたら、<br />
          <span style={{ color: "#FF6B35" }}>見積書</span> が出てくる。
        </h1>

        <p className="mt-6 max-w-2xl text-base sm:text-lg leading-relaxed font-light bp-rise" style={{ color: "#0B3D91", animationDelay: "0.15s" }}>
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-4 bp-rise" style={{ animationDelay: "0.45s" }}>
          <Link href="/signup" className="bp-cta inline-flex items-center gap-3 px-8 py-4 text-sm cursor-pointer">
            <span>無料ではじめる</span><span className="font-mono">→</span>
          </Link>
        ...
        <div className="mt-10 inline-flex items-center gap-3 px-4 py-2.5 font-mono text-[11px] tracking-wider bp-rise" style={{ border: "1.5px dashed #FF6B35", color: "#FF6B35", background: "rgba(255,107,53,0.04)", animationDelay: "0.6s" }}>
          <span className="font-bold">⚑ NOTICE</span>
          <span className="opacity-90" style={{ color: "#0B3D91" }}>現在ベータ期間中 — 全機能 無料 / クレカ不要</span>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: "#0B3D91" }}>
        職人の時間が、<br className="sm:hidden" />作業じゃないところで溶けていく。
      </h2>
          ...
          {steps.map(({ num, label, sub, desc }, i) => (
            <Reveal key={num} delay={i * 0.12}>
              <div className="relative flex items-start gap-5 sm:gap-7">
                <div className="relative z-10 flex h-14 w-14 sm:h-16 sm:w-16 shrink-0 items-center justify-center font-mono font-bold text-xl sm:text-2xl" style={{ background: "#0B3D91", color: "#F4F1E8", border: "2px solid #0B3D91" }}>
                  {num}
                </div>
...
function Features() {
  const features = [
    { num: "[01]", tag: "AI 図面解析", title: "AIが図面を読んで、材料を拾い出す", desc: "配管・電気・ダクト・建築など業種を指定するだけ。AIが必要な材料と数量を自動抽出。施工注意点や他業者との取り合い確認もまとめて。", bullets: ["材料・数量・単位の自動抽出", "施工上の注意事項をリスト化", "他業者との緩衝箇所を特定", "業種ごとに絞り込んで解析"] },
    { num: "[02]", tag: "単価マスタ", title: "会社の単価データを、クラウドで一元管理", desc: "自社の材料単価をクラウドに登録すれば、図面解析後に自動で金額計算。CSV で一括インポートもできるので既存データをそのまま移行。", bullets: ["CSV で既存データを一括インポート", "材料カテゴリで整理・検索", "解析結果と単価を自動連携", "チームで単価を共有・管理"] },
    { num: "[03]", tag: "見積書 PDF", title: "3タップで、プロ仕様の見積書 PDF", desc: "解析した材料一覧をそのまま見積書へ変換。単価マスタと連携して金額自動計算、A4印刷対応の PDF を即出力。お客様へその場で提出可能。", bullets: ["ワンクリックで見積書生成", "消費税込み・別途切替対応", "A4印刷対応の PDF 出力", "会社名・担当者・工事名を設定可能"] },
    { num: "[04]", tag: "チーム連携", title: "現場チームで、リアルタイムに共有", desc: "グループを作って招待コードを送るだけ。見積書・解析結果・メッセージを一カ所に集約。LINE の散らかりとお別れ。", bullets: ["招待コードで即参加", "グループチャットで一元化", "@メンションで確実に伝達", "見積・解析結果を全員で共有"] },
  ];
  return (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: "#0B3D91" }}>4つの機能仕様。</h2>
      <p className="mb-10 text-sm font-light max-w-2xl" style={{ color: "#0B3D91" }}>
...
function Edge() {
  const edges = [
    { num: "[E.1]", tag: "BUILT BY THE FIELD", title: "建設業界出身者が、現場の言葉で作る", desc: "外から見た想像じゃなく、内から見た「本当に困ってる」を解く。仕様の細部に建設の現実が宿る。" },
    { num: "[E.2]", tag: "ALL-IN-ONE", title: "拾い出し → 単価 → 見積書まで、一気通貫", desc: "他社は「解析だけ」「見積だけ」が多い中、TORU は最初から最後まで1つのアプリで完結。アプリ間の往復ゼロ。" },
    { num: "[E.3]", tag: "FAST ITERATION", title: "個人開発のフットワーク、要望は翌週リリース", desc: "大手 SaaS の半年待ちではなく、「現場の声 → 翌週には機能」のサイクル。あなたの意見が即、製品になる。" },
    { num: "[E.4]", tag: "BUILD IN PUBLIC", title: "ソースコードも開発過程も、全部公開", desc: "GitHub で全コード閲覧可能。何が動いているか、どう作られているか、すべて透明。信頼の前提が違う。" },
  ];
  return (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: "#0B3D91" }}>
        TORU が選ばれる、<br className="sm:hidden" />4つの理由。
      </h2>
      <p className="mb-10 text-sm font-light max-w-2xl" style={{ color: "#0B3D91" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {edges.map(({ num, tag, title, desc }, i) => (
          <Reveal key={num} delay={i * 0.08}>
            <div className="relative bg-white/65 p-6 sm:p-7 h-full" style={{ border: "1.5px solid #0B3D91", boxShadow: "4px 4px 0 rgba(255,107,53,0.18)" }}>
              <div className="flex items-baseline gap-3 mb-3">
                <span className="font-mono font-bold text-base" style={{ color: "#FF6B35" }}>{num}</span>
                <span className="font-mono text-[10px] tracking-widest opacity-60" style={{ color: "#0B3D91" }}>{tag}</span>
              </div>
...
function Pricing() {
  ...
  return (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: "#0B3D91" }}>シンプルな料金プラン。</h2>
      <p className="mb-8 text-sm font-light max-w-2xl" style={{ color: "#0B3D91" }}>
        4プラン展開。1人親方から法人まで、規模に応じて選べる設計。
      </p>
      <div className="mb-8 p-4 font-mono text-xs tracking-wider" style={{ border: "1.5px dashed #FF6B35", color: "#FF6B35", background: "rgba(255,107,53,0.04)" }}>
...
function FAQ() {
  ...
  return (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-10" style={{ color: "#0B3D91" }}>よくある質問</h2>
      <div className="space-y-3">
        {items.map(({ q, a }, i) => (
          <Reveal key={q} delay={i * 0.05}>
            <details className="group bg-white/55 p-5 cursor-pointer" style={{ border: "1.5px solid #0B3D91" }}>
              <summary className="list-none flex items-start gap-3">
                <span className="font-mono font-bold text-xs tracking-wider shrink-0 mt-0.5" style={{ color: "#FF6B35" }}>Q.{String(i + 1).padStart(2, "0")}</span>
...
function FinalCta() {
  return (
    <section className="mt-24 sm:mt-32 mb-24 text-center">
      ...
      <Reveal>
        <p className="font-mono text-[11px] tracking-widest mb-5 opacity-70" style={{ color: "#0B3D91" }}>START NOW — IT'S FREE DURING BETA</p>
        <h2 className="text-2xl sm:text-4xl font-bold mb-10 leading-tight" style={{ color: "#0B3D91" }}>
          今すぐ始めて、<br className="sm:hidden" />現場の手間を減らそう。
        </h2>
    ...
    <footer className="relative border-t-2 mt-8" style={{ borderColor: "#0B3D91" }}>
      <div className="mx-auto max-w-6xl px-5 sm:px-8 py-5">
        ...
        <a
          ...
          className="mb-5 flex items-center justify-between gap-3 p-4 font-mono text-[11px] tracking-wider transition hover:bg-[rgba(255,107,53,0.06)]"
          style={{ border: "1.5px dashed #FF6B35", color: "#FF6B35" }}
        >
          <span className="font-sans font-normal" style={{ color: "#0B3D91" }}>
            <strong className="font-bold" style={{ color: "#FF6B35" }}>⚑ アルファテスター募集中</strong> — フィードバックにご協力いただける方を募集しています
          </span>
          <span className="font-bold whitespace-nowrap">申込む →</span>
        </a>

        <div className="bp-titleblock">
          <div className="grid grid-cols-2 sm:grid-cols-4 text-[11px]">
            <Cell label="PROJECT" value="TORU" />
            <Cell label="SHEET" value="HOME" />
