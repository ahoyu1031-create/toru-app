...
**取込**: ①夜間wrapper（daily-brief.ps1）のBUILDプロンプトへ「意図文（公開アカウント向け・正確性=信用）」「act when ready」「最終ログ前に全主張をツール出力と突合」を追加 ②docs/fable-window-plan.mdに公式ガイド章を新設（採用/非採用の判断記録）③cc-best-practices-adoptedメモリへ追補8〜12。**意図的非採用**: 「旧モデル向け手順書型プロンプトはFableで品質低下→削れ」はOpusフォールバック互換（本プランの核心）を優先して維持。send_to_userツールも既存のログ＋トースト＋スタジオで代替済みのため不要と判断。既存運用（verify機械化・memory・難問先行）は公式推奨と一致確認＝答え合わせ完了。
