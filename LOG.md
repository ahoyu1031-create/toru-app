...
**根治（polish.mjs・全シリーズ共通）**: ①EQ緩和（300 -4→-2.5・3.2k +4→+2.5・9k +3→+1）②**deesser=0.45新設**（EQ後段・動的抑制）③コンプ2.5:1/-18dB/release180へ緩和 ④`--pitch/--tempo`のA/B用CLI上書き追加。**同一テイクA/B実測: シビランス上端-3.1dB・歯擦音ピークが本体より下へ・音量は不変**。明朝6時ビルドから自動適用。**未決=ピッチ+9%→+5%案（C）**: 帯域はBと同等・声のキャラが変わるため `output/_qa/voice-ab-20260710/` のA/B/C聴き比べでユーザー判断待ち（声=確定事項のため勝手に変えない）。今夜公開済みのbriefは旧チェーンのまま（公開済みは作り直さない方針）。
